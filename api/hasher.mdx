---
title: Hasher Interface
description: Core interface for computing perceptual hashes from images
---
...
<ParamField path="Calculate" type="func(image.Image) (hashtype.Hash, error)">
  Computes a perceptual hash from the provided image.
  
  <ParamField path="img" type="image.Image">
    The input image to hash. Accepts any type that implements the standard `image.Image` interface.
  </ParamField>
  
  <ResponseField name="hash" type="hashtype.Hash">
    The computed hash. The concrete type depends on the algorithm:
