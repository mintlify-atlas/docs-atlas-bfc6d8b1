...
<CardGroup cols={2}>
  <Card title="19 Hash Algorithms" icon="fingerprint">
...
imghash provides 19 perceptual hash algorithms, each optimized for different use cases:
...
- **BoVW (SimHash)** - Bag of Visual Words with SimHash encoding
...
- **BoVW (Histogram)** - Visual word histogram (Cosine distance)
- **BoVW (MinHash)** - Locality-sensitive hashing (Jaccard distance)
