...
description: 'Complete guide to the 9 distance and similarity functions for comparing perceptual hashes'
---
...
<CardGroup cols={2}>
  <Card title="Hash Types" icon="circle-check">
    Binary, UInt8, Float64 - works with all types
  </Card>
  
  <Card title="Complexity" icon="gauge">
    O(n) where n is hash length - very fast
  </Card>
  
  <Card title="Sensitivity" icon="arrows-up-down">
    Linear - proportional to element differences
  </Card>
  
  <Card title="Range" icon="ruler">
    [0, ∞) - unbounded maximum
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Binary Mode" icon="circle-nodes">
    Bitset intersection/union - standard Jaccard index
  </Card>
  
  <Card title="MinHash Mode" icon="hashtag">
  ...
  <Card title="Symmetric" icon="arrows-left-right">
...
<AccordionGroup>
  <Accordion title="Binary Hashes (Average, Difference, PHash)">
  ...
  <Accordion title="Histogram Features (ColorMoment, CLD, EHD)">
  ...
  <Accordion title="High-Dimensional Vectors (GIST, Zernike, HOGHash)">
  ...
  <Accordion title="Rotation-Variant Features (RadialVariance)">
